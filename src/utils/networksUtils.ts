...
export const deserializeNetworks = (networksJson: string): Map<string, SubstrateNetworkParams> => {
	const networksEntries = JSON.parse(networksJson);

	return new Map(networksEntries);
};

export const deepCopyNetworks = (networks: Map<string, SubstrateNetworkParams>): Map<string, SubstrateNetworkParams> =>
	deserializeNetworks(serializeNetworks(networks));

export const mergeNetworks = (defaultNetworks: Record<string, SubstrateNetworkParams>,
	newNetworksEntries: [string, SubstrateNetworkParams][]): Map<string, SubstrateNetworkParams> => {
